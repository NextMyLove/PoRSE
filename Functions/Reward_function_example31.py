...
def compute_reward(
    ...
    distance_temp: float = 0.1  # Increased sensitivity for distance reward
    velocity_temp: float = 0.2  # Reduced scale for velocity reward
    progress_temp: float = 0.5  # Adjusted for better intermediate progress
    time_penalty_temp: float = 0.05  # Increased scale for time penalty
    ...
    velocity_reward = torch.exp(-velocity_reward / velocity_temp) * 0.5  # Reduced scale
