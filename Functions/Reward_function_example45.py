...
def compute_reward(
    ...
    distance_temp: float = 0.01  # Increased sensitivity for distance reward
    velocity_temp: float = 0.5   # Adjusted to prevent dominance
    progress_temp: float = 0.1   # Adjusted for better intermediate progress
    time_penalty_temp: float = 0.5  # Increased scale for time penalty
    ...
    velocity_reward = torch.exp(velocity_reward / velocity_temp) * 0.1  # Re-scaled to a smaller range
