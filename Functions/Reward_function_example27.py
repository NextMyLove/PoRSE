...
def compute_reward(
    ...
    distance_temp: float = 0.02  # Increased sensitivity to distance
    velocity_temp: float = 0.5   # Reduced scale to prevent dominance
    progress_threshold: float = 0.5  # Threshold for significant progress
    time_penalty_temp: float = 0.05  # Increased to encourage faster task completion
    ...
    # Progress reward for significantly opening the cabinet door
    progress = cabinet_dof_pos[:, 3]  # Assuming the 4th DOF is the cabinet door
    progress_reward = torch.where(progress > progress_threshold, progress, torch.zeros_like(progress))
