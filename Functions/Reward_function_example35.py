...
def compute_reward(
    ...
    velocity_temp: float = 0.1  # Adjusted for cabinet door velocity
    ...
    door_velocity = torch.abs(cabinet_dof_vel[:, 3])  # Assuming the 4th DOF is the cabinet door
    velocity_reward = torch.exp(-door_velocity / velocity_temp)

    # Progress reward for opening the cabinet door (sigmoid to emphasize progress near the goal)
    door_progress = cabinet_dof_pos[:, 3]  # Assuming the 4th DOF is the cabinet door
    progress_reward = 1 / (1 + torch.exp(-(door_progress - 0.5) / progress_temp))  # Sigmoid function
