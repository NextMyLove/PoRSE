...
def compute_reward(
    ...
    distance_temp: float = 0.1  # Adjusted for better distance reward
    velocity_temp: float = 0.01  # Adjusted for better velocity reward
    progress_temp: float = 0.5  # Adjusted for better progress reward
    ...
    velocity_reward = torch.exp(velocity_reward / velocity_temp) * 0.1  # Rescaled to a smaller range
    ...
    progress_reward = torch.exp(progress_reward / progress_temp) * 3.0  # Increased scale
